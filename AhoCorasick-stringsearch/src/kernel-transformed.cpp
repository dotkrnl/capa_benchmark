...
static __dst_alloc_bucket_size_t __dst_alloc_bucket_for_request__dmemUi(__dst_alloc_size_t request)
{
...
static void __dst_alloc_list_push__dmemUi(__dst_alloc_bucket_size_t bucket,__dst_alloc_size_t entry)
{
  __dst_alloc_size_t prev = __dst_alloc_buckets__dmemUi[bucket] . prev;
  __dmemUi[entry] . _link . prev = prev;
  __dmemUi[entry] . _link . next = 0;
  if (prev == 0) 
    __dst_alloc_buckets__dmemUi[bucket] . next = entry;
   else 
    __dmemUi[prev] . _link . next = entry;
  __dst_alloc_buckets__dmemUi[bucket] . prev = entry;
}

static void __dst_alloc_list_remove__dmemUi(__dst_alloc_bucket_size_t bucket,__dst_alloc_size_t entry)
{
  __dst_alloc_size_t prev = __dmemUi[entry] . _link . prev;
  __dst_alloc_size_t next = __dmemUi[entry] . _link . next;
  if (prev == 0) 
    __dst_alloc_buckets__dmemUi[bucket] . next = next;
   else 
    __dmemUi[prev] . _link . next = next;
  if (next == 0) 
    __dst_alloc_buckets__dmemUi[bucket] . prev = prev;
   else 
    __dmemUi[next] . _link . prev = prev;
}

static __dst_alloc_size_t __dst_alloc_list_pop__dmemUi(__dst_alloc_bucket_size_t bucket)
{
...
static __dst_alloc_size_t __dst_alloc_index_for_node__dmemUi(__dst_alloc_size_t bucket_index,__dst_alloc_bucket_size_t bucket)
{
  return 1 + (bucket_index - (1 << bucket) + 1 << ARRAY_LOG - bucket);
}

static __dst_alloc_size_t __dst_alloc_node_for_index__dmemUi(__dst_alloc_size_t global_index,__dst_alloc_bucket_size_t bucket)
{
...
unsigned int __dst_alloc_malloc__dmemUi(__dst_alloc_size_t request)
{
  request = (request / sizeof(unsigned int ));
  if (request > (1 << ARRAY_LOG)) 
    return 0;
  __dst_alloc_bucket_size_t bucket = __dst_alloc_bucket_for_request__dmemUi(request);
...
void __dst_alloc_free__dmemUi(__dst_alloc_size_t ptr)
{
  if (ptr == 0) 
    return ;
  ptr -= 1;
  __dst_alloc_bucket_size_t bucket = __dst_alloc_bucket_for_request__dmemUi(__dmemUi[ptr] . _link . prev);
  __dst_alloc_size_t i = __dst_alloc_node_for_index__dmemUi(ptr,bucket);
  while(i != 0){
    __dst_alloc_flip_parent_split__dmemUi(i);
    if ((__dst_alloc_test_parent_split__dmemUi(i))) 
      break; 
...
;
struct __dst_alloc_list__dmemc __dmemc[1 << ARRAY_LOG];
__dst_alloc_size_t __dst_alloc_malloc__dmemc(unsigned int );
void __dst_alloc_free__dmemc(unsigned int );
typedef unsigned int __dst_alloc_size_t;
typedef unsigned char __dst_alloc_bucket_size_t;
unsigned char __dst_alloc_node_split__dmemc[(1 << ARRAY_LOG - 1) / 8];
...
struct __dst_alloc_list_base_t __dst_alloc_buckets__dmemc[ARRAY_LOG] = {{(1), (1)}};

static __dst_alloc_bucket_size_t __dst_alloc_bucket_for_request__dmemc(__dst_alloc_size_t request)
{
  __dst_alloc_bucket_size_t bucket = (ARRAY_LOG - 1);
  __dst_alloc_size_t size = 2;
  while(size < request){
...
static void __dst_alloc_list_push__dmemc(__dst_alloc_bucket_size_t bucket,__dst_alloc_size_t entry)
{
  __dst_alloc_size_t prev = __dst_alloc_buckets__dmemc[bucket] . prev;
  __dmemc[entry] . _link . prev = prev;
  __dmemc[entry] . _link . next = 0;
  if (prev == 0) 
    __dst_alloc_buckets__dmemc[bucket] . next = entry;
   else 
    __dmemc[prev] . _link . next = entry;
  __dst_alloc_buckets__dmemc[bucket] . prev = entry;
}

static void __dst_alloc_list_remove__dmemc(__dst_alloc_bucket_size_t bucket,__dst_alloc_size_t entry)
{
  __dst_alloc_size_t prev = __dmemc[entry] . _link . prev;
  __dst_alloc_size_t next = __dmemc[entry] . _link . next;
  if (prev == 0) 
    __dst_alloc_buckets__dmemc[bucket] . next = next;
   else 
    __dmemc[prev] . _link . next = next;
  if (next == 0) 
    __dst_alloc_buckets__dmemc[bucket] . prev = prev;
   else 
    __dmemc[next] . _link . prev = prev;
}

static __dst_alloc_size_t __dst_alloc_list_pop__dmemc(__dst_alloc_bucket_size_t bucket)
{
...
static __dst_alloc_size_t __dst_alloc_index_for_node__dmemc(__dst_alloc_size_t bucket_index,__dst_alloc_bucket_size_t bucket)
{
  return 1 + (bucket_index - (1 << bucket) + 1 << ARRAY_LOG - bucket);
}

static __dst_alloc_size_t __dst_alloc_node_for_index__dmemc(__dst_alloc_size_t global_index,__dst_alloc_bucket_size_t bucket)
{
  return (global_index - 1 >> ARRAY_LOG - bucket) + (1 << bucket) - 1;
}
...
__dst_alloc_size_t __dst_alloc_malloc__dmemc(__dst_alloc_size_t request)
{
  ...
  if (request > (1 << ARRAY_LOG)) 
    return 0;
  __dst_alloc_bucket_size_t bucket = __dst_alloc_bucket_for_request__dmemc(request);
  __dst_alloc_bucket_size_t original_bucket = bucket;
  while(((__dst_alloc_bucket_size_t )(bucket + 1)) != 0){
...
void __dst_alloc_free__dmemc(__dst_alloc_size_t ptr)
{
  if (ptr == 0) 
    return ;
  ptr -= 1;
  __dst_alloc_bucket_size_t bucket = __dst_alloc_bucket_for_request__dmemc(__dmemc[ptr] . _link . prev);
  __dst_alloc_size_t i = __dst_alloc_node_for_index__dmemc(ptr,bucket);
  while(i != 0){
    __dst_alloc_flip_parent_split__dmemc(i);
    if ((__dst_alloc_test_parent_split__dmemc(i))) 
      break; 
...
__dst_alloc_size_t __dst_alloc_malloc__dmemclass_node(unsigned int );
...
static __dst_alloc_bucket_size_t __dst_alloc_bucket_for_request__dmemclass_node(__dst_alloc_size_t request)
{
  __dst_alloc_bucket_size_t bucket = (ARRAY_LOG - 1);
  __dst_alloc_size_t size = 2;
  while(size < request){
...
static void __dst_alloc_list_push__dmemclass_node(__dst_alloc_bucket_size_t bucket,__dst_alloc_size_t entry)
{
  ...
  if (prev == 0) 
    __dst_alloc_buckets__dmemclass_node[bucket] . next = entry;
   else 
    __dmemclass_node[prev] . _link . next = entry;
  __dst_alloc_buckets__dmemclass_node[bucket] . prev = entry;
}

static void __dst_alloc_list_remove__dmemclass_node(__dst_alloc_bucket_size_t bucket,__dst_alloc_size_t entry)
{
  __dst_alloc_size_t prev = __dmemclass_node[entry] . _link . prev;
  __dst_alloc_size_t next = __dmemclass_node[entry] . _link . next;
  if (prev == 0) 
    __dst_alloc_buckets__dmemclass_node[bucket] . next = next;
   else 
    __dmemclass_node[prev] . _link . next = next;
  if (next == 0) 
    __dst_alloc_buckets__dmemclass_node[bucket] . prev = prev;
   else 
    __dmemclass_node[next] . _link . prev = prev;
}

static __dst_alloc_size_t __dst_alloc_list_pop__dmemclass_node(__dst_alloc_bucket_size_t bucket)
{
...
static __dst_alloc_size_t __dst_alloc_index_for_node__dmemclass_node(__dst_alloc_size_t bucket_index,__dst_alloc_bucket_size_t bucket)
{
  return 1 + (bucket_index - (1 << bucket) + 1 << ARRAY_LOG - bucket);
}

static __dst_alloc_size_t __dst_alloc_node_for_index__dmemclass_node(__dst_alloc_size_t global_index,__dst_alloc_bucket_size_t bucket)
{
  return (global_index - 1 >> ARRAY_LOG - bucket) + (1 << bucket) - 1;
}
...
__dst_alloc_size_t __dst_alloc_malloc__dmemclass_node(__dst_alloc_size_t request)
{
  ...
  if (request > (1 << ARRAY_LOG)) 
    return 0;
  __dst_alloc_bucket_size_t bucket = __dst_alloc_bucket_for_request__dmemclass_node(request);
  __dst_alloc_bucket_size_t original_bucket = bucket;
  while(((__dst_alloc_bucket_size_t )(bucket + 1)) != 0){
...
void __dst_alloc_free__dmemclass_node(__dst_alloc_size_t ptr)
{
  if (ptr == 0) 
    return ;
  ptr -= 1;
  __dst_alloc_bucket_size_t bucket = __dst_alloc_bucket_for_request__dmemclass_node(__dmemclass_node[ptr] . _link . prev);
  __dst_alloc_size_t i = __dst_alloc_node_for_index__dmemclass_node(ptr,bucket);
  while(i != 0){
    __dst_alloc_flip_parent_split__dmemclass_node(i);
    if ((__dst_alloc_test_parent_split__dmemclass_node(i))) 
      break; 
...
struct __rect_packed_type_L1058R__L1059R 
{
...
{
  volatile struct __rect_packed_type_L1058R__L1059R __rect_packed_var_L1058R__L1059R[1 << ARRAY_LOG];
  volatile unsigned int __rect_packed_top_L1058R__L1059R = 0U;
  __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _location = 1U;
  __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local0 = root;
  __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local1 = str;
  __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local2 = substring_index;
  ++__rect_packed_top_L1058R__L1059R;
  __rect_func_L0_L1058R__L1059R:
  if (0U == __rect_packed_top_L1058R__L1059R) 
    return __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _return;
  --__rect_packed_top_L1058R__L1059R;
  if (1U == __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _location) 
    goto __rect_func_L1_L1058R__L1059R;
  if (2U == __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _location) 
    goto __rect_func_L2_L1058R__L1059R;
  __rect_func_L1_L1058R__L1059R:
  __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local3 =  *(&(__dmemc + __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local1 + 0U - 1U) -> _data);
  if (__rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local3 == '%') {
    (&(__dmemclass_node + __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local0 + 0U - 1U) -> _data) -> substring_index = __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local2;
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _return = 0;
    goto __rect_func_L0_L1058R__L1059R;
  }
   else {
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local3 >= 'a' && __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local3 <= 'z'?(static_cast < void  >  (0)) : __assert_fail("ch >= 'a' && ch <= 'z'","kernel.cpp",47,__PRETTY_FUNCTION__);
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local4 = __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local3 - 'a';
    if (!(&(__dmemclass_node + __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local0 + 0U - 1U) -> _data) -> next[__rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local4]) {
      (&(__dmemclass_node + __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local0 + 0U - 1U) -> _data) -> next[__rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local4] = new_node();
      node_count += 1;
    }
    if (__rect_packed_top_L1058R__L1059R + 1U == 1 << ARRAY_LOG) 
g_fallback = true;
      0;
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _location = 2U;
    __rect_packed_var_L1058R__L1059R[1 + __rect_packed_top_L1058R__L1059R] . local2 = __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local2;
    __rect_packed_var_L1058R__L1059R[1 + __rect_packed_top_L1058R__L1059R] . local1 = __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local1 + ((unsigned int )1);
    __rect_packed_var_L1058R__L1059R[1 + __rect_packed_top_L1058R__L1059R] . local0 = (&(__dmemclass_node + __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local0 + 0U - 1U) -> _data) -> next[__rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . local4];
    ++__rect_packed_top_L1058R__L1059R;
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _location = 1U;
    goto __rect_func_L1_L1058R__L1059R;
    __rect_func_L2_L1058R__L1059R:
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . __rect_packed_type_L1058R__L1059R::_ret0 = (__rect_packed_var_L1058R__L1059R[1 + __rect_packed_top_L1058R__L1059R] . _return);
    __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . _return = __rect_packed_var_L1058R__L1059R[0 + __rect_packed_top_L1058R__L1059R] . __rect_packed_type_L1058R__L1059R::_ret0 + 1;
    goto __rect_func_L0_L1058R__L1059R;
  }
  goto __rect_func_L0_L1058R__L1059R;
}
...
struct __rect_packed_type_delete_tree___Fb_v_Gb__L4R__Fe___L1064R 
{
  __didxclass_node local0;
  int local1;
  unsigned int _location;
...
{
  volatile struct __rect_packed_type_delete_tree___Fb_v_Gb__L4R__Fe___L1064R __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[1 << ARRAY_LOG];
  volatile unsigned int __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R = 0U;
  __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . _location = 1U;
  __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local0 = root;
  ++__rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
  __rect_func_L0_delete_tree___Fb_v_Gb__L4R__Fe___L1064R:
  if (0U == __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R) 
    return ;
  --__rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
  if (1U == __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . _location) 
    goto __rect_func_L1_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
  if (2U == __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . _location) 
    goto __rect_func_L2_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
  __rect_func_L1_delete_tree___Fb_v_Gb__L4R__Fe___L1064R:
  if (!__rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local0) {
    goto __rect_func_L0_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
  }
  for (__rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local1 = 0; __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local1 < 26; __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local1++) {
    if (__rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R + 1U == 1 << ARRAY_LOG) 
g_fallback = true;
      0;
    __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . _location = 2U;
    __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[1 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local0 = (&(__dmemclass_node + __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local0 + 0U - 1U) -> _data) -> next[__rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local1];
    ++__rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
    __rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . _location = 1U;
    goto __rect_func_L1_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
    __rect_func_L2_delete_tree___Fb_v_Gb__L4R__Fe___L1064R:
    0;
  }
  __dst_alloc_free__dmemclass_node(__rect_packed_var_delete_tree___Fb_v_Gb__L4R__Fe___L1064R[0 + __rect_packed_top_delete_tree___Fb_v_Gb__L4R__Fe___L1064R] . local0);
  goto __rect_func_L0_delete_tree___Fb_v_Gb__L4R__Fe___L1064R;
}
extern "C" {
/*
