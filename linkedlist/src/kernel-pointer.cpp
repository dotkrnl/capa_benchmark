...
# 73 "<stdin>"
unsigned char __dst_alloc_node_split__dmemL105R[(1 << (10 - 1)) / 8];
...
struct __dst_alloc_list_base_t __dst_alloc_buckets__dmemL105R[10] = {{1, 1}};
...
{
  __didxL105R temp;
  if ((&(__dmemL105R + ptr + 0U - 1U) -> _data) -> next == 0L) {
    (&(__dmemL105R + ptr + 0U - 1U) -> _data) -> next = previous;
    return ptr;
  }
   else {
    temp = reverse_rec((&(__dmemL105R + ptr + 0U - 1U) -> _data) -> next,ptr);
    (&(__dmemL105R + ptr + 0U - 1U) -> _data) -> next = previous;
    return temp;
  }
}
