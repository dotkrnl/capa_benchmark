...
int main(int argc, char** argv)
{
  ...
  #define TARGET_DEVICE GET_STRING(SDX_PLATFORM)
    ...
    int device_results[5*DATA_SIZE];    // results returned from device
    int host_results[5*DATA_SIZE];      // results returned from host
    ...
    //
    cl_event readevent;
    err = clEnqueueReadBuffer( commands, output, CL_TRUE, 0, sizeof(int) * 5 * DATA_SIZE, device_results, 0, NULL, &readevent );
    if (err != CL_SUCCESS)
        {
